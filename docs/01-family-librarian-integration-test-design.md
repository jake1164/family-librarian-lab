...
| BASE-04 | Backup/restore round trip | Run scripts/backups/create-backup.sh and restore-backup.sh, unmodified, against the real deployment; a request created after the backup is reverted away, and a request created before it survives -- proving the scripts' PostgreSQL round trip, not just that pg_dump/pg_restore exit 0. CWA/Audiobookshelf directory backup is scripts/backups' own concern, covered directly in family-librarian's tests/scripts/*.test.sh. |
