...
4. `./lab up --ep <name>` -- brings up the base profile plus this provider's overlay. Checks out the provider's source (if `repo_url` is set), refreshing it to the latest commit on its current branch on every run, then builds its image and waits for the stack to be healthy.
